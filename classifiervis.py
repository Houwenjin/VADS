...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True, final_classifier=None, netVis=None, netATT=None, dec_size=4096, dec_hidden_size=4096,_att=None):
        ...
        if self.netVis:
            ...
            self.input_dim = self.input_dim#+2048 #+ dec_size
            # self.input_dim = self.input_dim + dec_hidden_size
            #self.input_dim = self.input_dim + dec_hidden_size+ dec_size
            self.model =  LINEAR_LOGSOFTMAX_CLASSIFIER(self.input_dim, self.nclass)
            self.train_X = self.compute_fear_out(self.train_X, self.train_Y,self.att,self.input_dim)
            
            self.test_unseen_feature = self.compute_fear_out(self.test_unseen_feature,self.test_unseen_label,self.att,self.input_dim)
            self.test_seen_feature = self.compute_fear_out(self.test_seen_feature, self.test_seen_label,self.att,self.input_dim)
    ...
    def compute_fear_out(self, test_X, test_Y, att, new_size):
        start = 0
        ntest = test_X.size()[0]
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                with torch.no_grad():
                    inputX = Variable(test_X[start:end].cuda())
                    inputY = test_Y[start:end].cuda()
                   
                    #inputATT =att[inputY].cuda()
            else:
                with torch.no_grad():
                    inputX = Variable(test_X[start:end])
                    inputY = test_Y[start:end]
                   
                    #inputATT =att[inputY]

            #_,_,_,_, _, feat2 = self.netFR(inputX)
            _,_,feat2,feat3 = self.netVis(inputX)
            # feat_att = self.netATT(inputATT)


            #feat1 = self.netFR.getLayersOutDet()
            # new_test_X[start:end] = torch.cat([inputX,feat1],dim=1).data.cpu()
            #new_test_X[start:end] = torch.cat([inputX,feat3],dim=1).data.cpu()
            #
            # new_test_X[start:end] = torch.cat([inputX,feat_att],dim=1).data.cpu()
            #new_test_X[start:end] = torch.cat([inputX,inputATT],dim=1).data.cpu()
            #new_test_X[start:end] = torch.cat([inputX,feat2,feat_att],dim=1).data.cpu()
